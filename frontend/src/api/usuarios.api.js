...
export async function listarCatalogosUsuarios() {
  const res = await http.get("/api/usuarios/catalogos");
  return {
...
export async function crearUsuario(payload) {
  const res = await http.post("/api/usuarios", payload);
  return res?.data;
}
...
export function actualizarUsuario(id_usuario, payload) {
  ...
  return http.put(`/api/usuarios/${id_usuario}`, payload);
}

// PATCH /api/usuarios/:id/estado
export async function cambiarEstadoUsuario(id_usuario, id_estado_usuario) {
  const res = await http.patch(`/api/usuarios/${id_usuario}/estado`, { id_estado_usuario });
  return res?.data;
}

// POST /api/usuarios/:id/reset-password
export async function resetPasswordUsuario(id_usuario, password) {
  const res = await http.post(`/api/usuarios/${id_usuario}/reset-password`, { password });
  return res?.data;
}
